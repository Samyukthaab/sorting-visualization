...
class DrawInformation:
	...
	def __init__(self, width, height, lst):
		...
		pygame.display.set_caption("Visualization of sorting algorithms")
		self.set_list(lst)
	...
	for i in range(len(lst) - 1):
		for j in range(len(lst) - 1 - i):
			...
			if (num1 > num2 and ascending) or (num1 < num2 and not ascending):
				...
				draw_list(draw_info, {j: draw_info.GREEN, j + 1: draw_info.LAV}, True)
				yield True
...
def insertion_sort(draw_info, ascending=True):
	...
	for i in range(1, len(lst)):
		...
		while True:
			...
			i = i - 1
			lst[i] = current
			draw_list(draw_info, {i - 1: draw_info.GREEN, i: draw_info.LAV}, True)
			yield True
